...
import { z } from 'zod';
import { Loader2, User, FileSpreadsheet } from 'lucide-react';
...
export function KeteranganUmumForm() {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <div className="space-y-6 rounded-md border p-4 md:p-6 bg-white shadow-sm">
          <div className="flex items-center gap-2 border-b pb-2">
            ...
            <h3 className="text-lg font-semibold">Keperluan & Identitas (Surat Keterangan)</h3>
          </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="md:col-span-2">
                  ...
                  <FormControl><Input placeholder="Contoh: Keterangan Beda Nama / Ahli Waris" {...field} disabled={isSubmitting} /></FormControl>
                  <FormDescription>Jelaskan isi pokok keterangan yang ingin Anda buat.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
        ...
        <Button type="submit" size="lg" disabled={isSubmitting} className="w-full">
          {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : 'Ajukan Surat Keterangan Umum'}
        </Button>
      </form>
    </Form>
  );
