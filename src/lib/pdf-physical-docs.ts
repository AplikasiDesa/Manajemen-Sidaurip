...
export const generatePhysicalDocPDF = async (type: string, data: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  if (type === 'prop_cover') {
    ...
    doc.setFontSize(16);
    doc.text("PEMERINTAH DESA SIDAURIP", pageWidth / 2, 230, { align: "center" });
    doc.setFontSize(14);
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    const pendahuluan = `Dalam rangka meningkatkan infrastruktur dan kualitas hidup masyarakat di ${data.lokasi}, Pemerintah Desa Sidaurip memandang perlu melaksanakan ${data.namaKegiatan} melalui pemanfaatan ${data.sumberDana} Tahun Anggaran ${data.tahunAnggaran}.`;
    ...
    y += 45;
    doc.setFont("helvetica", "bold");
    doc.text("Menyetujui,\nKepala Desa Sidaurip", pageWidth / 2, y, { align: "center" });
    y += 25;
    doc.text((data.nama_kades || "TASIMIN").toUpperCase(), pageWidth / 2, y, { align: "center" });
    const nW = doc.getTextWidth((data.nama_kades || "TASIMIN").toUpperCase());
    ...
    const renderSurveyPage = (pageIdx: number) => {
        ...
        doc.setFontSize(10);
        doc.text("DESA", margin, 25); doc.text(`: SIDAURIP`, margin + 30, 25);
        ...
        doc.setFontSize(10);
        y += 10;
        doc.text(`Sidaurip, ${dStr}`, sigCenterX, y, { align: "center" });
        y += 6;
    ...
    docL.setFont("helvetica", "normal");
    docL.text(`Desa Sidaurip, ${dStr}`, sX[2], y, { align: "center" });
    ...
    docL.text("TASIMIN", sX[1], y + 10, { align: "center" });
    docL.line(sX[1] - docL.getTextWidth("TASIMIN")/2, y + 10.8, sX[1] + docL.getTextWidth("TASIMIN")/2, y + 10.8);
    ...
    const openingY = 85;
    const openingText = `Pada hari ini ${format(dBast, "EEEE", { locale: localeID })} tanggal ${terbilang(getDate(dBast))} bulan ${format(dBast, "MMMM", { locale: localeID })} tahun ${terbilang(getYear(dBast))} bertempat di Desa Sidaurip, kami yang bertanda tangan dibawah ini :`;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    ...
    const alamat1 = `Desa Sidaurip Kecamatan Gandrungmangu yang selanjutnya disebut PIHAK KESATU`;
    ...
    doc.text("Nama", labelX, y); doc.text(":", dotX, y); doc.setFont("helvetica", "bold"); doc.text("TASIMIN", valX, y); y += 6;
    doc.setFont("helvetica", "normal");
    doc.text("Jabatan", labelX, y); doc.text(":", dotX, y); doc.text("Pemegang Kekuasaan Pengelolaan Keuangan Desa", valX, y); y += 6;
    doc.text("Alamat", labelX, y); doc.text(":", dotX, y); 
    const alamat2 = `Desa Sidaurip Kecamatan Gandrungmangu yang selanjutnya disebut PIHAK KEDUA`;
    ...
    y += 25;
    doc.text(`Sidaurip, ${format(dBast, "d MMMM yyyy", { locale: localeID })}`, pageWidth - margin, y, { align: "right" }); y += 10;
    addSignatures(y, "PIHAK KEDUA\nPemegang Kekuasaan\nPengelolaan Keuangan Desa", "TASIMIN", "PIHAK KESATU\nPelaksana Kegiatan Anggaran", data.nama_kasi);
  }
    ...
    doc.setFontSize(10);
    let y = 62;
    const bodyText = `Bahwa dalam pelaksanaan pembangunan Desa Sidaurip Kecamatan Gandrungmangu Kabupaten Cilacap Provinsi Jawa Tengah, maka pada :`;
    ...
    doc.text("Kepala Desa Sidaurip", pageWidth / 2, y, { align: "center" }); y += 18;
    doc.text("TASIMIN", pageWidth / 2, y, { align: "center" });
    ...
    y += 30;
    doc.setFont("helvetica", "normal");
    doc.text(`Sidaurip, ${formattedDate}`, pageWidth - margin - 60, y);
    y += 6;
    doc.setFont("helvetica", "bold");
    doc.text("Ketua TPK Desa Sidaurip,", pageWidth - margin - 60, y);
    y += 28;
