...
export const addKopSuratSync = (doc: jsPDF, img: HTMLImageElement | null, margin: number, pageWidth: number) => {
    ...
    doc.text("Jl. Perintis No.144, Desa Sidaurip, Kec. Gandrungmangu, Kab. Cilacap, Jawa Tengah", pageWidth / 2 + 10, 31, { align: "center" });
    doc.text("Tlp. 0851-3221-7144, Laman :  www.sidaurip.gandrungmangu.id, Pos-el : sidaurip818@gmail.com", pageWidth / 2 + 10, 35, { align: "center" });
    doc.setFont("helvetica", "bold");
...
export const generateBASTPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  const addIdentitas = (num: string, name: string, job: string, role: string) => {
    ...
    doc.text(":", dotX, currentY);
    const alamatFull = `Desa Sidaurip Kecamatan Gandrungmangu yang selanjutnya disebut ${role}`;
  ...
  addIdentitas("II", "TASIMIN", "Pemegang Kekuasaan Pengelolaan Keuangan Desa", "PIHAK KEDUA");
  currentY += 2;
  ...
  doc.setFont("helvetica", "bold");
  doc.text("TASIMIN", margin + 35, nameY, { align: "center" });
...
export const generateSuratTugasPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  const drawDasar = () => {
    ...
    const items = values.dasar ? values.dasar.split("\n").filter(Boolean) : [
          ...
          "Peraturan Desa Sidaurip Nomor 08 Tahun 2025 tentang Anggaran Pendapatan dan Belanja Desa (APBDes) Tahun Anggaran 2026.",
        ];
...
const drawUntuk = () => {
  ...
  const drawSignature = () => {
    ...
    currentY += 28;
    doc.text("TASIMIN", sigX, currentY);
    const width = doc.getTextWidth("TASIMIN");
...
export const generateSPPDPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  addRow("1", "Pejabat berwenang yang memberi perintah", "Kepala Desa Sidaurip");
  ...
  addRow("6", "a. Tempat Berangkat", "Desa Sidaurip");
  ...
  addRow("9", "Pembebanan Anggaran", "APBDes Desa Sidaurip 2026");
  addRow("", "a. Instansi", "Pemerintah Desa Sidaurip");
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Sidaurip,", sigX, currentY + 12);
  currentY += 32;
  doc.text("TASIMIN", sigX, currentY);
  const nW = doc.getTextWidth("TASIMIN");
  doc.line(sigX, currentY + 1, sigX + nW, currentY + 1);
  ...
  doc.text("Berangkat dari : Desa Sidaurip", midX + 7, margin + 5);
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Sidaurip,", midX + colW2 / 2, margin + 27, { align: "center" });
  doc.text("TASIMIN", midX + colW2 / 2, margin + 40, { align: "center" });
  const wH1 = doc.getTextWidth("TASIMIN");
...
let y2 = margin + rowH2;
    ...
    doc.text(":", rDotX, y2 + 5);
    doc.text("Desa Sidaurip", rValueX, y2 + 5);
    ...
    doc.text("Desa Sidaurip", ivValueX, y2 + 5);
    doc.setFontSize(8);
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Sidaurip,", margin + colW2 / 2, y2 + 25, { align: "center" });
  doc.text("TASIMIN", margin + colW2 / 2, y2 + 38, { align: "center" });
  const wH2 = doc.getTextWidth("TASIMIN");
