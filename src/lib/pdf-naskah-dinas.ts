...
export const generateNaskahPDF = async (type: string, data: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  if (type === 'UND') {
    ...
    doc.text(data.nomorSurat || "..... / No / 03 / 2026", margin + 28, currentY);
    ...
    doc.setFont("helvetica", "normal");
    doc.text(`Sidaurip, ${format(letterDate, "d MMMM yyyy", { locale: localeID })}`, pageWidth - margin, currentY, { align: "right" });
    ...
    const details = [
      ...
      { l: "Tempat", v: data.tempat || "Balai Desa Sidaurip" },
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.text("KEPUTUSAN KEPALA DESA SIDAURIP", pageWidth / 2, currentY, { align: "center" });
    currentY += lineH;
    doc.text(`NOMOR : ${data.nomorSurat || "..... / No / 03 / 2026"}`, pageWidth / 2, currentY, { align: "center" });
    currentY += 8;
    ...
    doc.text("KEPALA DESA SIDAURIP,", pageWidth / 2, currentY, { align: "center" });
    currentY += 10;
    ...
    const opening = `Pada hari ini ${dayName} tanggal ${terbilang(dateNum)} bulan ${monthName} tahun ${terbilang(yearNum)}, bertempat di ${data.tempat || "Balai Desa Sidaurip"}, telah dilaksanakan ${data.title || "kegiatan"}.`;
  ...
  if (type !== 'UND') {
    doc.text(`Sidaurip, ${footerDateStr}`, sigX, currentY);
    currentY += 5;
  }
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Sidaurip,", sigX, currentY);
  currentY += 18; 
