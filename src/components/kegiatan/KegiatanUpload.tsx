...
export function KegiatanUpload({ onSuccess, initialData }: { onSuccess?: () => void, initialData?: any }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      location: initialData?.location || "Balai Desa Sidaurip",
  ...
  const handleSelectAgenda = (agenda: AgendaItem) => {
    ...
    form.setValue("location", agenda.location || "Balai Desa Sidaurip", { shouldDirty: true, shouldValidate: true })
  ...
  const handleAskAI = async () => {
    ...
    try {
      const response = await generateNotulen({
        title: values.title,
        location: values.location || "Balai Desa Sidaurip",
